...
export default function StaffDashboard() {
  ...
  const fetchPrescriptions = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/staff/prescriptions`, {
        headers: {
  ...
  const updatePrescriptionStatus = async (prescriptionId, newStatus) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/staff/prescriptions/${prescriptionId}`, {
        method: "PATCH",
        headers: {
